...
export default function MenuItem({ name, image, id, attributes, prices, isSpicy, isVegan }: IMenuItemProps) {
  ...
  return (
    <li className="w-full list-none">
      <Link to={`/product/${id}`}>
        <div className="flex max-h-[170px] rounded-2xl border-1 border-border-black/10 pr-4 transition-all ease-in-out hover:bg-accent-light dark:border-dark-separation-line dark:hover:bg-dark-separation-line md:min-h-[170px] md:pr-8">
          ...
          <div className="mt-2 flex flex-col items-end justify-end py-4 md:py-7">
            {oldPrice ? (
              <span className="justify-self-end text-sm text-text-grey line-through md:text-base">$ {oldPrice}</span>
            ) : null}
            <h3 className="mt-1 text-base font-semibold text-text-dark dark:text-primary lg:text-lg">$ {corePrice}</h3>
            <AddToCartBtn />
          </div>
        </div>
      </Link>
